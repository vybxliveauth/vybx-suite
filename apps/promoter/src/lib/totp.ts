/**
 * Minimal RFC 6238 TOTP using Web Crypto API — works in browser and Node.
 * Matches the backend implementation in src/common/totp.ts.
 *
 * Secret format: base64url-encoded raw bytes (20 bytes from randomBytes(20)).
 */
...
async function computeTotp(keyBuf: ArrayBuffer, counter: number): Promise<string> {
  ...
  // SHA-1 HMAC is always 20 bytes; offset is 0-15; offset+3 ≤ 18 — always safe
...
export async function generateTotp(secret: string): Promise<string> {
  ...
  return computeTotp(keyBuf, counter);
}
